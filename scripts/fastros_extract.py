...
#   9 = slowest / smallest
#
# Using 1 gives considerably faster image extraction than high compression.
#
# Image pixel values are NOT changed by this setting.
# Only compression level changes.
# ---------------------------------------------------------------------------
...
# FIXED.
#
# The original camera encoding remains:
#
#     bayer_bggr8
#
# We directly convert BGGR -> BGR.
# ---------------------------------------------------------------------------

BAYER_ENCODING = "bayer_bggr8"
...
# ============================================================================
# FAST BAYER BRIGHTNESS NORMALISATION
# ============================================================================
...
def normalise_bayer_brightness(
    ...
    """
    Fast brightness correction for uint8 Bayer images.

    IMPORTANT:
        - Bayer pattern remains BGGR.
        - No demosaicing occurs here.
        - No colour/channel rearrangement occurs here.
        - Dark images are never brightened.
        - Only excessively bright images are darkened.

    Optimisation:
        Instead of np.percentile(), use a 256-bin histogram.

    Since the image is uint8, there are only 256 possible intensity values.
    Therefore a histogram is sufficient to determine the percentile.
    ...
    if (
        not ENABLE_BAYER_NORMALISATION
        or
        percentile_value <= TARGET_BRIGHTNESS
    ):
    ...
    # Apply correction
    #
    # Only allocate a new float/uint8 array if correction is actually needed.
    # ------------------------------------------------------------------------
...
def debayer_and_undistort(
    ...
    """
    Fast camera processing pipeline:
    ...
    this version removes several unnecessary copies/conversions.

    The Bayer encoding remains bayer_bggr8.
    """
    ...
    # ------------------------------------------------------------------------
    #
    # bytes() creates a Python bytes object.
    # np.frombuffer then creates a NumPy view over that memory.
    #
    # No explicit image copy is made here.
    # ------------------------------------------------------------------------
...
def main():
    ...
    print(
        "INTENSITY-AWARE + FAST BAYER IMAGE PROCESSING"
    )
    print("=" * 80)
    ...
    print()
    print("Bayer encoding:")
    print(f"  {BAYER_ENCODING}")

    print()
    print("Image processing optimisations:")
    print("  Direct Bayer -> BGR: YES")
    print("  RGB/BGR/RGB conversions removed: YES")
    print("  PIL removed: YES")
    print("  OpenCV PNG writer: YES")
    print("  Histogram brightness estimation: YES")
    print(f"  PNG compression level: {PNG_COMPRESSION}")

    print()
    print("Brightness normalisation:")
    print(
    ...
    print("\n" + "=" * 80)
    print(
        "PASS 2: FAST RAW BAYER -> NORMALISE -> DEBAYER -> RECTIFY"
    )
    print("=" * 80)
    ...
    last_report_time = image_start_time
    ...
    with Reader(BAG_PATH) as reader:
        ...
        for connection, bag_timestamp, raw in reader.messages():
            ...
            if (
                ...
                print(
                    f"Images: "
                    f"{total_new_images} processed | "
                    f"{rate:.2f} images/s | "
                    f"last 100: "
                    f"{100 / max(interval, 1e-6):.2f} images/s"
                )
    ...
    print()
    print(
        f"Image processing finished in "
        f"{image_elapsed:.2f} seconds."
    )

    if total_new_images > 0:

        print(
            f"Average image throughput: "
            f"{total_new_images / max(image_elapsed, 1e-6):.2f} images/s"
        )
    ...
    print(
        f"  Original encoding: {BAYER_ENCODING}"
    )
